...
hf_model = LlamaForCausalLM.from_pretrained(weight_path, torch_dtype=torch.float16).to(DEVICE)
assert type(hf_model) == LlamaForCausalLM

prompt = f"Content: {very_long_text}\n\n Summary:"
prompt_ids = tokenizer(prompt, return_tensors="pt")['input_ids'].view(1, -1).to(DEVICE)#type:ignore
max_new_tokens = 128
...
del hf_model, prompt_ids, gen_config

engine = Engine(weight_path, DEVICE)
llama_output = engine.execute([prompt], temperature=0.001)[0]
