...
class Engine:
    ...
    def execute(self, prompts: List[str], max_new_tokens: int = 50, temperature: float = 0.001) -> List[str]:
        prompt_ids = self.tokenizer(prompts, return_tensors="pt", padding=True)
        input_ids = prompt_ids['input_ids'].to(self.model.device)
        attention_mask = prompt_ids['attention_mask'].to(self.model.device)
        output_ids = temperature_sampling_generate(self.model, input_ids, max_new_tokens, temperature, attention_mask)
